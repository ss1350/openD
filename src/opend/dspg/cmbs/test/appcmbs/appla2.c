...
//#define PRINT APP_LA_INFO_PRINT
#define PRINT if (0) APP_LA_INFO_PRINT

#define REMOVE_MSB(x) ((x) & 0x7F)
#define ADD_MSB(x) ((x) | 0x80)

static u8 popcount(u8 u8_byte)
{
    u8 u8_index, u8_result = 0;
    for ( u8_index = 0; u8_index < 8; ++u8_index )
    {
        u8_result += (u8_byte >> u8_index) & 0x01;
    }
...
{
    /* first 4 bytes are reserved for Entry Id, Entry Length (each can consume up to 2 bytes) */
    u16 u16_entry_len = u16_index - 4;
...
E_CMBS_RC cmbs_util_LA_Fld_DateTimeAdd( u8 * pv_buff, ST_UTIL_LA_FLD_DATETIME* pst_LA_Fld_DateTime , u16 *pindex )
{
    u16 index_len;
    ...
    {
        pv_buff[(*pindex)++] = ((pst_LA_Fld_DateTime->u8_Year / 10) << 4) + (pst_LA_Fld_DateTime->u8_Year % 10);
        pv_buff[(*pindex)++] = ((pst_LA_Fld_DateTime->u8_Month / 10) << 4) + (pst_LA_Fld_DateTime->u8_Month % 10);
        pv_buff[(*pindex)++] = ((pst_LA_Fld_DateTime->u8_Day / 10) << 4) + (pst_LA_Fld_DateTime->u8_Day % 10);
    }
    if ( (pst_LA_Fld_DateTime->u8_Coding == CMBS_TIME) || (pst_LA_Fld_DateTime->u8_Coding == CMBS_DATE_TIME) )
    {
        pv_buff[(*pindex)++] = ((pst_LA_Fld_DateTime->u8_Hours / 10) << 4) + (pst_LA_Fld_DateTime->u8_Hours % 10);
        pv_buff[(*pindex)++] = ((pst_LA_Fld_DateTime->u8_Mins / 10) << 4) + (pst_LA_Fld_DateTime->u8_Mins % 10);
        pv_buff[(*pindex)++] = ((pst_LA_Fld_DateTime->u8_Secs / 10) << 4) + (pst_LA_Fld_DateTime->u8_Secs % 10);
        pv_buff[(*pindex)++] = ((pst_LA_Fld_DateTime->u8_TimeZone / 10) << 4) + (pst_LA_Fld_DateTime->u8_TimeZone % 10);
    }
...
E_CMBS_RC         cmbs_util_LA_Fld_LineIdAdd( u8 * pv_buff, ST_UTIL_LA_FLD_LINE_ID * pst_LA_Fld_LineId , u16 *pindex )
{
    ...
    u16_LengthLocalIndex = (*pindex)++;  // assuming only 1 byte
    pv_buff[(*pindex)++]  = ADD_MSB(pst_LA_Fld_LineId->u8_Attribute);
    pv_buff[(*pindex)++]  = pst_LA_Fld_LineId->Subtype;
    if ( pst_LA_Fld_LineId->Subtype != CMBS_UTIL_LA_LINE_ID_ALL_LINES_RELATED_TO )
    {
...
// Building Corresponding table;
    ...
    u8_NumOfHs = popcount(st_LA_Fld_AttachedHandsets->pu8_Data[0]);
    pv_buff[(*pindex)++]  = ADD_MSB(u8_NumOfHs);  // number of handsets
...
E_CMBS_RC cmbs_util_LA_Fld_AttachedHandsetsGet(u8 * pv_buff, ST_UTIL_LA_FLD_ATTACHED_HANDSETS* st_LA_Fld_AttachedHandsets)
{
    ...
    st_LA_Fld_AttachedHandsets->u8_FieldId      = pu8_Buffer[0];
    st_LA_Fld_AttachedHandsets->u16_DataLen     = 1;
    st_LA_Fld_AttachedHandsets->u8_Attribute    = REMOVE_MSB(pu8_Buffer[2]);
    pu8_Buffer[4]                               = REMOVE_MSB(pu8_Buffer[4]);                              
    st_LA_Fld_AttachedHandsets->pu8_Data        = &pu8_Buffer[4];
...
E_CMBS_RC cmbs_util_LA_Fld_CFNAAdd(  u8 * pv_buff, ST_UTIL_LA_FLD_CFNA *st_LA_Fld_CFNA,  u16 *pindex )
{
    ...
    if ( !(st_LA_Fld_CFNA -> u8_NotSetByUSer) )
    {
        ...
        pv_buff[(*pindex)++]  = st_LA_Fld_CFNA->u8_Timeout;
...
E_CMBS_RC cmbs_util_LA_Fld_CFNAGet(  u8 * pv_buff, ST_UTIL_LA_FLD_CFNA *st_LA_Fld_CFNA)
{
    ...
    st_LA_Fld_CFNA->u8_Timeout      = pu8_Buffer[u16_index++];
...
{
    return cmbs_util_LA_Fld_Type4_Add( pv_buff, st_LA_Fld_OKStatus, CMBS_UTIL_LA_FLD_OK_STATUS, pindex  );
}

E_CMBS_RC	cmbs_util_LA_Fld_OKStatusGet( u8 * pv_buff, ST_UTIL_LA_FLD_OK_STATUS *st_LA_Fld_OKStatus )
{
    return cmbs_util_LA_Fld_Type4_Get( pv_buff, st_LA_Fld_OKStatus, CMBS_UTIL_LA_FLD_INTRUSION_CALL_MODE );
}

E_CMBS_RC	cmbs_util_LA_Fld_LineUseStatusAdd(  u8 * pv_buff, ST_UTIL_LA_FLD_LINE_USE_STATUS*st_LA_Fld_LineUseStatus, u16 *pindex )
{
    return cmbs_util_LA_Fld_Type4_Add( pv_buff, st_LA_Fld_LineUseStatus, CMBS_UTIL_LA_FLD_LINE_USE_STATUS, pindex  );
}

E_CMBS_RC	cmbs_util_LA_Fld_LineUseStatusGet(  u8 * pv_buff, ST_UTIL_LA_FLD_LINE_USE_STATUS *st_LA_Fld_LineUseStatus )
{
    return cmbs_util_LA_Fld_Type4_Get( pv_buff, st_LA_Fld_LineUseStatus, CMBS_UTIL_LA_FLD_INTRUSION_CALL_MODE );
}
...
E_CMBS_RC	cmbs_util_LA_Fld_CallForwardingStatusAdd(  u8 * pv_buff, ST_UTIL_LA_FLD_CALL_FWRD_STATUS*st_LA_Fld_CallForwarding, u16 *pindex )
{
    u16 u16_LengthLocalIndex;
    pv_buff[(*pindex)++]  = st_LA_Fld_CallForwarding->u8_FieldId;
    u16_LengthLocalIndex = (*pindex)++;  
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_CallForwarding->u8_Attribute);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_CallForwarding->u8_CFUStatus);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_CallForwarding->u8_CFNAStatus);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_CallForwarding->u8_CFBStatus);

    pv_buff[u16_LengthLocalIndex] = ADD_MSB(*pindex - u16_LengthLocalIndex - 1); 
    return CMBS_RC_OK;
}

E_CMBS_RC	cmbs_util_LA_Fld_CallForwardingStatusGet( u8 * pv_buff, ST_UTIL_LA_FLD_CALL_FWRD_STATUS *st_LA_Fld_CallForwarding )
{
    st_LA_Fld_CallForwarding->u8_FieldId    = pv_buff[0];
    st_LA_Fld_CallForwarding->u8_Attribute  = REMOVE_MSB(pv_buff[2]);
    st_LA_Fld_CallForwarding->u8_CFUStatus  = REMOVE_MSB(pv_buff[3]);
    st_LA_Fld_CallForwarding->u8_CFNAStatus = REMOVE_MSB(pv_buff[4]);
    st_LA_Fld_CallForwarding->u8_CFBStatus  = REMOVE_MSB(pv_buff[5]);
	return CMBS_RC_OK;
}
...
{
    u16 u16_LengthLocalIndex;
    pv_buff[(*pindex)++]  = st_LA_fld_DiagErrorStatus->u8_FieldId;
    u16_LengthLocalIndex = (*pindex)++;  // assuming only 1 byte
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_fld_DiagErrorStatus->u8_Attribute);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_fld_DiagErrorStatus->u8_ErrorType);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_fld_DiagErrorStatus->u8_ErrorNumber);
...
{
    st_LA_fld_DiagErrorStatus->u8_FieldId     = pv_buff[0];
    st_LA_fld_DiagErrorStatus->u8_Attribute   = REMOVE_MSB(pv_buff[2]);
    st_LA_fld_DiagErrorStatus->u8_ErrorType   = REMOVE_MSB(pv_buff[3]);
    st_LA_fld_DiagErrorStatus->u8_ErrorNumber = REMOVE_MSB(pv_buff[4]);
	return CMBS_RC_OK;
}
...
{
    return cmbs_util_LA_Fld_Type4_Add( pv_buff, st_LA_fld_DTAMId, CMBS_UTIL_LA_FLD_DTAM_ID, pindex  );
}

E_CMBS_RC   cmbs_util_LA_Fld_DTAMIdGet(u8 * pv_buff, ST_UTIL_LA_FLD_DTAM_ID *st_LA_fld_DTAMId)
{
    return cmbs_util_LA_Fld_Type4_Get( pv_buff, st_LA_fld_DTAMId, CMBS_UTIL_LA_FLD_DTAM_ID );
}
...
{
    u16 u16_LengthLocalIndex;
    pv_buff[(*pindex)++]  = st_LA_fld_DTAMActivation->u8_FieldId;
    u16_LengthLocalIndex = (*pindex)++;  
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_fld_DTAMActivation->u8_Attribute);
    pv_buff[(*pindex)++]  = st_LA_fld_DTAMActivation->u8_DTAMActivated;
    pv_buff[(*pindex)++]  = st_LA_fld_DTAMActivation->u8_DTAMTimeout;
...
{
    st_LA_fld_DTAMActivation->u8_FieldId= pv_buff[0];
    st_LA_fld_DTAMActivation->u8_Attribute = REMOVE_MSB(pv_buff[2]);
    st_LA_fld_DTAMActivation->u8_DTAMActivated = pv_buff[3];
    st_LA_fld_DTAMActivation->u8_DTAMTimeout = pv_buff[4];
	return CMBS_RC_OK;
}
...
{
    return cmbs_util_LA_Fld_Type4_Add( pv_buff, st_LA_fld_DTAMWelcomeMsgParams, CMBS_UTIL_LA_FLD_WELCOME_MSG_PARAMETERS, pindex  );
}
...
{
    return cmbs_util_LA_Fld_Type4_Get( pv_buff, st_LA_fld_DTAMWelcomeMsgParams, CMBS_UTIL_LA_FLD_WELCOME_MSG_PARAMETERS );
}
...
{
    return cmbs_util_LA_Fld_Type2_Add( pv_buff, st_LA_fld_DTAMPositionIdx, CMBS_UTIL_LA_FLD_POSITION_IDX, pindex );
}

E_CMBS_RC cmbs_util_LA_Fld_DTAMPositionIdxGet(u8 * pv_buff, ST_UTIL_LA_FLD_POSITION_IDX *st_LA_fld_DTAMPositionIdx)
{
    return cmbs_util_LA_Fld_Type2_Get( pv_buff, st_LA_fld_DTAMPositionIdx, CMBS_UTIL_LA_FLD_POSITION_IDX );
}
...
{
    u16 u16_LengthLocalIndex;
    pv_buff[(*pindex)++]  = st_LA_fld_DTAMMsgDuration->u8_FieldId;
    u16_LengthLocalIndex = (*pindex)++;  
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_fld_DTAMMsgDuration->u8_Attribute);
	pv_buff[(*pindex)++] = st_LA_fld_DTAMMsgDuration->u8_DurationHours;
    pv_buff[(*pindex)++]  = st_LA_fld_DTAMMsgDuration->u8_DurationMinutes;
    pv_buff[(*pindex)++]  = st_LA_fld_DTAMMsgDuration->u8_DurationSeconds;
...
{
    st_LA_fld_DTAMMsgDuration->u8_FieldId= pv_buff[0];
    st_LA_fld_DTAMMsgDuration->u8_Attribute = REMOVE_MSB(pv_buff[2]);
    st_LA_fld_DTAMMsgDuration->u8_DurationHours = pv_buff[3];
    st_LA_fld_DTAMMsgDuration->u8_DurationMinutes = pv_buff[4];
    st_LA_fld_DTAMMsgDuration->u8_DurationSeconds = pv_buff[5];
	return CMBS_RC_OK;
}

E_CMBS_RC cmbs_util_LA_Fld_SMSIdAdd(u8 * pv_buff, ST_UTIL_LA_FLD_SMS_ID *st_LA_fld_SMSId, u16 *pindex)
{
    return cmbs_util_LA_Fld_Type4_Add( pv_buff, st_LA_fld_SMSId, CMBS_UTIL_LA_FLD_SMS_ID, pindex  );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSIdGet(u8 * pv_buff, ST_UTIL_LA_FLD_SMS_ID *st_LA_fld_SMSId)
{
    return cmbs_util_LA_Fld_Type4_Get( pv_buff, st_LA_fld_SMSId, CMBS_UTIL_LA_FLD_SMS_ID );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSEnableAdd(u8 * pv_buff, ST_UTIL_LA_SMS_ENABLE *st_LA_fld_SMSEnable, u16 *pindex)
{
    return cmbs_util_LA_Fld_Type2_Add( pv_buff, st_LA_fld_SMSEnable, CMBS_UTIL_LA_FLD_ENABLE_SMS, pindex );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSEnableGet(u8 * pv_buff, ST_UTIL_LA_SMS_ENABLE *st_LA_fld_SMSEnable)
{
    return cmbs_util_LA_Fld_Type2_Get( pv_buff, st_LA_fld_SMSEnable, CMBS_UTIL_LA_FLD_ENABLE_SMS );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSMaxSizeAdd(u8 * pv_buff, ST_UTIL_LA_FLD_MAX_SMS_SIZE *st_LA_fld_SMSMaxSize, u16 *pindex)
{
    return cmbs_util_LA_Fld_Type2_Add( pv_buff, st_LA_fld_SMSMaxSize, CMBS_UTIL_LA_FLD_ENABLE_SMS, pindex );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSMaxSizeGet(u8 * pv_buff, ST_UTIL_LA_FLD_MAX_SMS_SIZE *st_LA_fld_SMSMaxSize)
{
    return cmbs_util_LA_Fld_Type2_Get( pv_buff, st_LA_fld_SMSMaxSize, CMBS_UTIL_LA_FLD_ENABLE_SMS );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSCServerAdd(  u8 * pv_buff, ST_UTIL_LA_FLD_SMSC_SERVER *st_LA_Fld_BlockedNumber,  u16 *pindex )
{
    return cmbs_util_LA_Fld_Type3_Add( pv_buff, st_LA_Fld_BlockedNumber, CMBS_UTIL_LA_FLD_SMSC_SERVER, pindex  );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSCServerGet(  u8 * pv_buff, ST_UTIL_LA_FLD_SMSC_SERVER *st_LA_Fld_BlockedNumber)
{
    return cmbs_util_LA_Fld_Type3_Get( pv_buff, st_LA_Fld_BlockedNumber, CMBS_UTIL_LA_FLD_SMSC_SERVER );
}
...
{
    return cmbs_util_LA_Fld_Type2_Add( pv_buff, st_LA_fld_SMSDeliveryReport, CMBS_UTIL_LA_FLD_SMS_DELIVERY_REPORT, pindex );
}
...
{
    return cmbs_util_LA_Fld_Type2_Get( pv_buff, st_LA_fld_SMSDeliveryReport, CMBS_UTIL_LA_FLD_SMS_DELIVERY_REPORT );
}
...
{
    return cmbs_util_LA_Fld_Type2_Add( pv_buff, st_LA_fld_SMSValidityPeriod, CMBS_UTIL_LA_FLD_SMS_VALIDITY_PERIOD, pindex );
}
...
{
    return cmbs_util_LA_Fld_Type2_Get( pv_buff, st_LA_fld_SMSValidityPeriod, CMBS_UTIL_LA_FLD_SMS_VALIDITY_PERIOD );
}
...
{
    return cmbs_util_LA_Fld_Type3_Add( pv_buff, st_LA_Fld_SMSAllowedEncoding, CMBS_UTIL_LA_FLD_ALLOWED_ENCODING, pindex  );
}

E_CMBS_RC cmbs_util_LA_Fld_SMSAllowedEncodingGet(  u8 * pv_buff, ST_UTIL_LA_FLD_ALLOWED_ENCODING *st_LA_Fld_SMSAllowedEncoding)
{
    return cmbs_util_LA_Fld_Type3_Get( pv_buff, st_LA_Fld_SMSAllowedEncoding, CMBS_UTIL_LA_FLD_ALLOWED_ENCODING );
}
E_CMBS_RC cmbs_util_LA_Fld_SMSSendRequestAdd( u8 * pv_buff, ST_UTIL_SEND_REQUEST * st_LA_Fld_SendReq , u16 *pindex )
{
    return cmbs_util_LA_Fld_Type1_Add( pv_buff, st_LA_Fld_SendReq, CMBS_UTIL_LA_FLD_SMS_SEND_REQUEST, pindex  );
}
E_CMBS_RC cmbs_util_LA_Fld_SMSSendRequestGet( u8 * pv_buff, ST_UTIL_SEND_REQUEST * st_LA_Fld_SendReq )
{
    return cmbs_util_LA_Fld_Type1_Get( pv_buff, st_LA_Fld_SendReq, CMBS_UTIL_LA_FLD_SMS_SEND_REQUEST );
}
E_CMBS_RC cmbs_util_LA_Fld_NetworkSideEncAdd( u8 * pv_buff, ST_UTIL_NETWORK_SIDE_ENC * st_LA_Fld_SendReq , u16 *pindex )
{
    u16 u16_LengthLocalIndex;
    pv_buff[(*pindex)++]  = st_LA_Fld_SendReq->u8_FieldId;
    u16_LengthLocalIndex = (*pindex)++; 
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_SendReq->u8_Attribute);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_SendReq->u8_UsedNetworkSideEnc);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_SendReq->u8_UsedEncVariant_1);
    pv_buff[(*pindex)++]  = ADD_MSB(st_LA_Fld_SendReq->u8_UsedEncVariant_2);
    pv_buff[u16_LengthLocalIndex] = ADD_MSB(*pindex - u16_LengthLocalIndex - 1); // Add the length now
    return CMBS_RC_OK;
}
E_CMBS_RC cmbs_util_LA_Fld_NetworkSideEncGet( u8 * pv_buff, ST_UTIL_NETWORK_SIDE_ENC * st_LA_Fld_SendReq )
{
    st_LA_Fld_SendReq->u8_FieldId     = pv_buff[0];
    st_LA_Fld_SendReq->u8_Attribute   = REMOVE_MSB(pv_buff[2]);
    st_LA_Fld_SendReq->u8_UsedNetworkSideEnc = REMOVE_MSB(pv_buff[3]);
    st_LA_Fld_SendReq->u8_UsedEncVariant_1 = REMOVE_MSB(pv_buff[4]);
    st_LA_Fld_SendReq->u8_UsedEncVariant_2 = REMOVE_MSB(pv_buff[5]);
    return CMBS_RC_OK;
}
E_CMBS_RC cmbs_util_LA_Fld_SMSSizeAdd( u8 * pv_buff, ST_UTIL_SMS_SIZE * st_LA_Fld_SMSSize , u16 *pindex )
{
    return cmbs_util_LA_Fld_Type4_Add( pv_buff, st_LA_Fld_SMSSize, CMBS_UTIL_LA_FLD_SMS_SIZE, pindex  );
}
E_CMBS_RC cmbs_util_LA_Fld_SMSSizeGet( u8 * pv_buff, ST_UTIL_SMS_SIZE * st_LA_Fld_SMSSize )
{
    return cmbs_util_LA_Fld_Type4_Get( pv_buff, st_LA_Fld_SMSSize, CMBS_UTIL_LA_FLD_SMS_SIZE );
}
E_CMBS_RC cmbs_util_LA_Fld_SMSContentGet( u8 * pv_buff, ST_UTIL_LA_FLD_SMS_CONTENT * st_LA_Fld_SMSContent )
{
    return cmbs_util_LA_Fld_Type3_Get( pv_buff, st_LA_Fld_SMSContent, CMBS_UTIL_LA_FLD_SMS_CONTENT );
}
E_CMBS_RC cmbs_util_LA_Fld_SMSContentAdd( u8 * pv_buff, ST_UTIL_LA_FLD_SMS_CONTENT * st_LA_Fld_SMSContent , u16 *pindex )
{
    return cmbs_util_LA_Fld_Type3_Add( pv_buff, st_LA_Fld_SMSContent, CMBS_UTIL_LA_FLD_SMS_CONTENT, pindex  );
}
/*****************************************************************************
 * List Access fields
 *****************************************************************************/
                                              ...
                                              u16 *pindex )
{
    ...
    if ( e_LATYPE != CMBS_UTIL_LA_FLD_NEW &&
         ...
         e_LATYPE != CMBS_UTIL_LA_FLD_SMS_SEND_REQUEST)
    {
        //    APP_LA_INFO_PRINT( "cmbs_util_LA_Fld_Type1_Add ERROR: IE_TYPE %d no ST_UTIL_LA_FLD_TYPE_1\n", e_LATYPE );
...
E_CMBS_RC         cmbs_util_LA_Fld_Type1_Get( u8 * pv_buff, ST_UTIL_LA_FLD_TYPE_1 * pst_LA_Fld_Type1, E_CMBS_UTIL_LA_TYPE type )
{
    ...
    if ( type == CMBS_UTIL_LA_FLD_NEW ||
         ...
         type == CMBS_UTIL_LA_FLD_SMS_SEND_REQUEST)
    {
                                              ...
                                              u16 *pindex  )
{
    ...
    if ( e_LATYPE != CMBS_UTIL_LA_FLD_ASSOC_MDY &&
         ...
         e_LATYPE != CMBS_UTIL_LA_FLD_OK_STATUS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_LINE_USE_STATUS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_WELCOME_MSG_PARAMETERS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_POSITION_IDX &&
         e_LATYPE != CMBS_UTIL_LA_FLD_ENABLE_SMS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_SMS_VALIDITY_PERIOD &&
         e_LATYPE != CMBS_UTIL_LA_FLD_SMS_DELIVERY_REPORT)
    {
        //   APP_LA_INFO_PRINT( "cmbs_util_LA_Fld_Type2_Add ERROR: IE_TYPE %d no ST_UTIL_LA_FLD_TYPE_2\n", e_LATYPE );
        return CMBS_RC_ERROR_PARAMETER;
    }
...
E_CMBS_RC         cmbs_util_LA_Fld_Type2_Get( u8 * pv_buff, ST_UTIL_LA_FLD_TYPE_2 * pst_LA_Fld_Type2, E_CMBS_UTIL_LA_TYPE type  )
{
    ...
    if ( type == CMBS_UTIL_LA_FLD_ASSOC_MDY ||
         ...
         type == CMBS_UTIL_LA_FLD_WELCOME_MSG_PARAMETERS ||
         type == CMBS_UTIL_LA_FLD_POSITION_IDX ||
         type == CMBS_UTIL_LA_FLD_ENABLE_SMS ||
         type == CMBS_UTIL_LA_FLD_SMS_VALIDITY_PERIOD||
         type == CMBS_UTIL_LA_FLD_SMS_DELIVERY_REPORT)
    {
                                              ...
                                              u16 *pindex  )
{
    ...
    if ( e_LATYPE != CMBS_UTIL_LA_FLD_NAME       &&
         ...
         e_LATYPE != CMBS_UTIL_LA_FLD_HS_USE_STATUS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_CALL_FRWRD_STATUS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_SMSC_SERVER &&
         e_LATYPE != CMBS_UTIL_LA_FLD_ALLOWED_ENCODING &&
         e_LATYPE != CMBS_UTIL_LA_FLD_SMS_CONTENT)   
    {
        //   APP_LA_INFO_PRINT( "cmbs_util_LA_Fld_Type3_Add ERROR: IE_TYPE %d no ST_UTIL_LA_FLD_TYPE_3\n", e_LATYPE );
        return CMBS_RC_ERROR_PARAMETER;
    }
...
E_CMBS_RC cmbs_util_LA_Fld_Type3_Get( u8 * pv_buff, ST_UTIL_LA_FLD_TYPE_3 * pst_LA_Fld_Type3 , E_CMBS_UTIL_LA_TYPE type )
{
    ...
    if ( type == CMBS_UTIL_LA_FLD_NAME       ||
         ...
         type == CMBS_UTIL_LA_FLD_HS_USE_STATUS ||
         type == CMBS_UTIL_LA_FLD_CALL_FRWRD_STATUS ||
         type == CMBS_UTIL_LA_FLD_ALLOWED_ENCODING  ||
         type == CMBS_UTIL_LA_FLD_SMSC_SERVER ||
         type == CMBS_UTIL_LA_FLD_SMS_CONTENT)
   {
                                        ...
                                        u16 *pindex  )
{
    ...
    if ( e_LATYPE != CMBS_UTIL_LA_FLD_OK_STATUS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_LINE_USE_STATUS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_WELCOME_MSG_PARAMETERS &&
         e_LATYPE != CMBS_UTIL_LA_FLD_DTAM_ID &&
         e_LATYPE != CMBS_UTIL_LA_FLD_SMS_ID &&
         e_LATYPE != CMBS_UTIL_LA_FLD_SMS_SIZE)
    {
        return CMBS_RC_ERROR_PARAMETER;
    }

    pv_buff[(*pindex)++]  = pst_LA_Fld_Type4->u8_FieldId;
    pv_buff[(*pindex)++]  = ADD_MSB(2);  // Length

    pv_buff[(*pindex)++]  = ADD_MSB(pst_LA_Fld_Type4->u8_Attribute);
    pv_buff[(*pindex)++]  = ADD_MSB(pst_LA_Fld_Type4->u8_Value);
...
E_CMBS_RC	cmbs_util_LA_Fld_Type4_Get( u8 * pv_buff, ST_UTIL_LA_FLD_TYPE_4 * pst_LA_Fld_Type4, E_CMBS_UTIL_LA_TYPE type  )
{
    ...
    if ( type != CMBS_UTIL_LA_FLD_OK_STATUS &&
         type != CMBS_UTIL_LA_FLD_LINE_USE_STATUS &&
         type != CMBS_UTIL_LA_FLD_WELCOME_MSG_PARAMETERS &&
         type != CMBS_UTIL_LA_FLD_DTAM_ID &&
         type != CMBS_UTIL_LA_FLD_SMS_ID &&
         type != CMBS_UTIL_LA_FLD_SMS_SIZE)
    {
        pst_LA_Fld_Type4->u8_FieldId	= pu8_Buffer[0];
        pst_LA_Fld_Type4->u8_Attribute  = REMOVE_MSB(pu8_Buffer[2]);
        pst_LA_Fld_Type4->u8_Value      = REMOVE_MSB(pu8_Buffer[3]);
                                             ...
                                             u16 *pu16_index)
{
    ...
    *pu8_length= pu8_Buffer[*pu16_index];
    if ( *pu8_length )
    {
        *ppu8_data = pu8_Buffer+(++(*pu16_index));
