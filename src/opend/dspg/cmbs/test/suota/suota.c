...
static char CMBS_SUOTA_LOCAL_FILE_NAME[MAX_FILE_NAME_LENGTH] = "ITCM_H";
...
// decalre and initialize SUOTA modes
t_st_SUOTAFlags g_SUOTAFlags =
{
    FALSE,   //bool b_HeaderCreatedbyHost;
    FALSE,   //bool b_FileDownload;
    FALSE,   // bool b_EarlyChunkPrepare;
    0        // u32 u32_AbortSessionId;
};
...
void   cplane_ProcessHandUrlInd(t_st_GmepSuotaUrlInd *, u16);
...
int GetFileName(char *pRequestBuffer);
#ifdef SUOTA_SDU_CHOPPING    
void suota_CalculateByteRange(t_Suota_GmepSessionTbl *pSession);
#else
void suota_CalculateByteRange(void);
#endif
...
unsigned long suota_quitThread(void)
{
...
void suota_push_mode_send(void)
{
    gmep_us_PushModeSend();
#if 0
...
int suota_ReadFile(u32 u32_NumOfBytes, u32 *u32_Offset)
{
    ...
    if ( fseek(fd, u32_NumOfBytesRead, SEEK_SET) == (-1) )
        return -1;
    if ( u32_NumOfBytesRead < g_st_FileInfo.u32_FileSize )
    {
        ...
        // check that with the new u32_NumOfBytesRead we did not reach the end of the file
        // if yes, close the file
        // printf("\n suota_ReadFile sets u32_NumOfBytesRead to %d\n", u32_NumOfBytesRead);        
        if ( u32_NumOfBytesRead == g_st_FileInfo.u32_FileSize )
        {
            fclose(fd);
            fd = NULL;
...
{
    // This function construct the message to be sent to the HS
    u32 u32_count = 0;
    int resp;

#ifdef SUOTA_SDU_CHOPPING
    if (!pSession->SDUChoppingHS)
    {
        st_Response->content_length = (g_st_FileInfo.u32_To - g_st_FileInfo.u32_From + 1);
}
else
{
    st_Response->content_length = pSession->u32_To - pSession->u32_From + 1;
}
#else
        st_Response->content_length = (g_st_FileInfo.u32_To - g_st_FileInfo.u32_From + 1);
#endif

         // printf(" suota_ConstructMessage: From %d to %d length %d\n", g_st_FileInfo.u32_From, g_st_FileInfo.u32_To, st_Response->content_length);
    ...
    {
        SUOTA_INFO("calling suota_ReadFile() with length = %d\n", st_Response->content_length);
        resp = suota_ReadFile(st_Response->content_length, (u32 *)&(st_Response->responseBuff[u32_count]));
        if ( resp <= 0 )
        {
            SUOTA_ERROR("Could not read from file!! ERROR = %d\n", resp);
        }
        st_Response->responseBuffLen = st_Response->content_length;
    }
}
...
#ifdef SUOTA_SDU_CHOPPING    
//        suotaSessionTbl[i].u8_SequenceNum = 0;
        suotaSessionTbl[i].SDUChoppingHS  = 0;
        suotaSessionTbl[i].ContinueSend   = 0;
#endif
    }
                ...
                case GMEP_US_SESSION_CREATE_ACK:
                    {
                        ...
                        // Target sends indicator if handset supports SDU chopping in the 4th byte of the buffer.
#ifdef SUOTA_SDU_CHOPPING    
                        g_st_FileInfo.u8_SequenceNum = 0x80;  // reset Sequence number for start
#endif
                        //suota_CallBack(GMEP_US_SESSION_CREATE_ACK, NULL, sessionId);
                        // pGmepMsgResp->buf[3] contains SDU chopping indication of the handset
                        suota_CallBack(GMEP_US_SESSION_CREATE_ACK, &pGmepMsgResp->buf[3], pGmepMsgResp->appSessionId);
                        break;
                    }
...
void suota_PrintHex(u8 *buf, u32 len)
{
    u32 i;
    SUOTA_INFO("\nFile content starts \n");
    for (i = 0; i < len; i++) SUOTA_INFO(" %x ", *(buf + i));
    SUOTA_INFO("\nFile content ends \n");
}
...
void suota_PrintChar(u8 *buf, u32 len)
{
    u32 i;
    SUOTA_INFO("\n");
    for (i = 0; i < len; i++) SUOTA_INFO(" %c ", *(buf + i));
    SUOTA_INFO("\n");
}
    ...
    case GMEP_US_SESSION_CREATE_ACK:
        {
            ...
            if ( pSession != NULL )
            {
                ...
                if (pData != NULL)
                {
                    u8 *u8_SUCHopping = (u8 *)pData;
                    pSession->SDUChoppingHS = *u8_SUCHopping;
                }
#endif           
    ...
    case GMEP_US_SESSION_CLOSE_ACK:
        {
            ...
            u8_FirstTransDone = 0;
            memset(&g_st_FileInfo, 0, sizeof(ST_IE_SUOTA_FILE_INFO));
            //The following line deletes the SUOTA Local File upon SUOTA completion.
            ...
            for (i = 0; i < GMEP_MAX_SESSIONS; i++)
            {
                if ( (suotaSessionTbl[i].sesStatus >= SUOTA_GMEP_SESSION_ACTIVE) && (suotaSessionTbl[i].sessionId == (s32)sessionid) )
                {
                    ...
                    //If file download option is not used - closing not existing socket cause host crush
    ...
    case GMEP_US_DATA_RECV:
        {
            ...
            for (i = 0; i < GMEP_MAX_SESSIONS; i++)
            {
                if ( (suotaSessionTbl[i].sesStatus == SUOTA_GMEP_SESSION_DOWNLOAD) && (suotaSessionTbl[i].sessionId == (s32)sessionid) )
                {
                    ...
                    threadMsgReq.appId = SUOTA_APPLICATION_ID;
...
#endif
{
    // For the option: g_SUOTAFlags.b_EarlyChunkPrepare = TRUE --> range is not read from HS request (because we did not receive it yet at this stage) but calculated manually

    u32 from = 0, to = 0;

    // printf("\n suota_CalculateByteRange - u32_NumOfBytesRead = %d\n", u32_NumOfBytesRead);

    if ( u32_NumOfBytesRead < g_st_FileInfo.u32_FileSize )
    {
        from = u32_NumOfBytesRead;
        if ( !g_SUOTAFlags.b_HeaderCreatedbyHost )
        {
            to = from + (MAX_GMEP_SDU_SIZE)-1;
            if ( to > g_st_FileInfo.u32_FileSize )
            {
                to = g_st_FileInfo.u32_FileSize - 1;
#ifdef SUOTA_SDU_CHOPPING    
                // last segment
                g_st_FileInfo.u8_SequenceNum &= ~0x80;  // clear MSB in Sequence number                  
                pSession->ContinueSend = 0;     
#endif                
                
            }
#ifdef SUOTA_SDU_CHOPPING    

            // printf("\n suota_CalculateByteRange to=%d  u32_To=%d \n", to, pSession->u32_To);

            if (pSession->SDUChoppingHS)
            {
                    if (to >= g_st_FileInfo.u32_To)
                    {
                        // Handset didn't yet request more bytes, stop sending segments
                        to = g_st_FileInfo.u32_To;
                        // last segment of actual request from handset
                        g_st_FileInfo.u8_SequenceNum &= ~0x80;  // clear MSB in Sequence number         
                        pSession->ContinueSend = 0;     
                    }
            }
...
#endif                           
            SUOTA_INFO("SUOTA: suota_CalculateByteRange NEW range from<%d>  to<%d>\n", from, to);
        }
        else
        {
            to = from + (SUOTA_MAX_PAYLOAD_SIZE)-1;
            if ( to > g_st_FileInfo.u32_FileSize )
            {
                to = g_st_FileInfo.u32_FileSize - 1;
            }
...
#endif
    {
        // printf(" suota_CalculateByteRange stores from %d to %d, ContinueSend = %d\n", from, to, pSession->ContinueSend);
        g_st_FileInfo.u32_From = from;
        g_st_FileInfo.u32_To = to;
    }
#ifdef SUOTA_SDU_CHOPPING    
    else
    {
        // printf(" suota_CalculateByteRange stores from %d to %d, ContinueSend = %d\n", from, to, pSession->ContinueSend);
        pSession->u32_From = from;
        pSession->u32_To   = to;
    }
 ...
 */
#ifdef SUOTA_SDU_CHOPPING    
int modifyRangeHttpHdr(char *pRequestBuffer, t_Suota_GmepSessionTbl *pSession)
#else
int modifyRangeHttpHdr(char *pRequestBuffer)
#endif
{
    // Read request from HS and modify requested range.
    // Possible options:
    // 1. g_SUOTAFlags.b_HeaderCreatedbyHost = TRUE --> range is read from HS request and modifyed according to SUOTA_MAX_PAYLOAD_SIZE
    // 2. g_SUOTAFlags.b_HeaderCreatedbyHost = FALSE --> range is read from HS request and modifyed according to MAX_GMEP_SDU_SIZE (to leave enough space for header)
    char	*pRangeStart = 0;
    char 	*pReqPtr = 0;
    //char	*pToBytePtr=0;
    int  	ret = -1;
    u32 	from = 0, to = 0, fromLength = 0, Recvd_to, Recvd_from; 
    char	newRange[100];
    char	fromStr[10];
    int		inx;
    int		oldRangeSize, newRangeSize;
#ifdef SUOTA_SDU_CHOPPING    
    bool    ContinueSendPrevious = pSession->ContinueSend;
#endif
    ...
    if ( pRangeStart )
    {
        ...
        while (*pReqPtr != '\r' && *pReqPtr != '\n') // loop until range ends
        {
            ...
            {
                fromStr[inx] = 0;
                from = (u32)atoi(fromStr);
                fromLength = (u32)strlen(fromStr);
                inx = 0;
            }
            else
                inx++;
        ...
        // save received values for later use (storage after recalculation)
        Recvd_to = to;
        Recvd_from = from;

#ifdef SUOTA_SDU_CHOPPING    
        // To is always to be set
        pSession->u32_To = Recvd_to;
        // printf(" modifyRangeHttpHdr max stored To = %d \n", Recvd_to);
#endif           

        SUOTA_INFO("\nSUOTA: modifyRangeHttpHdr from<%d>  to<%d>\n", from, to);
        oldRangeSize = fromLength + strlen(fromStr);
		if (b_suota_local == TRUE)
		{
			if (!g_SUOTAFlags.b_HeaderCreatedbyHost)
			{
#ifdef SUOTA_SDU_CHOPPING    
// the RECV might be late received and doesn't contain the latest increase
                     if (from < u32_NumOfBytesRead)
                     {
                         from = u32_NumOfBytesRead;
                     }
#endif

				// If the header is created by the Target, the range shall be maximum SDU size
				if (to - from > (MAX_GMEP_SDU_SIZE))
				{
#ifdef SUOTA_SDU_CHOPPING    
                    pSession->ContinueSend = 1;     
#endif                
					to = from + (MAX_GMEP_SDU_SIZE)-1;
					if (to > g_st_FileInfo.u32_FileSize)
					{
                ...
                if (to > Recvd_to)
                {
                    // restrict it to received to from handset
                   to = Recvd_to;
                }
#endif                
					// printf("SUOTA: modifyRangeHttpHdr NEW range from<%d>  to<%d> continueSend=%d\n", from, to, pSession->ContinueSend);
					SUOTA_INFO("SUOTA: modifyRangeHttpHdr NEW range from<%d>  to<%d>\n", from, to);				
			}
			else
			{
				// If Host creates the header, range shall be smaller, to enable addition of the header
				if (to - from > (SUOTA_MAX_PAYLOAD_SIZE))
				{
					to = from + (SUOTA_MAX_PAYLOAD_SIZE)-1;
					if (to > g_st_FileInfo.u32_FileSize)
					{
		...
		{
			// We assume that the header that we get from the HTTP server is not greater than MAX_GMEP_SDU_SIZE/2
...
#endif           
            {
#ifndef SUOTA_SDU_CHOPPING
				g_st_FileInfo.u32_From = from;
				g_st_FileInfo.u32_To = to;
#else //SUOTA_SDU_CHOPPING
                // used for FROM TO for full SDU 
			    g_st_FileInfo.u32_From = Recvd_from;
			    g_st_FileInfo.u32_To     = Recvd_to;
                 // used for actual SDU sequence (for SDUChoppingHS)
                 pSession->u32_From = from;            
                 pSession->u32_To     = to;

                 // printf("modifyRangeHttpHdr - SDU chopping - FileInfo: From %d to %d - Session From %d to %d\n",
                 // g_st_FileInfo.u32_From, g_st_FileInfo.u32_To, pSession->u32_From, pSession->u32_To);
#endif
            }
#ifdef SUOTA_SDU_CHOPPING    
            else 
            {
                if (!ContinueSendPrevious && (to > g_st_FileInfo.u32_To))
                {
			        g_st_FileInfo.u32_From = from;
			        g_st_FileInfo.u32_To     = to;
                }
                ...
                {
                    pSession->ContinueSend = 0;     
                    return ret; // return -1 for not sending a packet
                }
            }
#endif
        }
...
int GetFileName(char *pRequestBuffer)
{
    ...
    int		NameLen = 0;
    int RetVal = -1;
    char *pNameString = "Host: http://", *pITCMHPtr;
    pNamePtr = strstr(pRequestBuffer, pNameString);

    // printf("\nGetFileName\n");

    // check for "ITCM_H" case
    pITCMHPtr = strstr(pRequestBuffer, "ITCM_H");
    if (pITCMHPtr)
    {
        strncpy((char *)&CMBS_SUOTA_LOCAL_FILE_NAME, pITCMHPtr, strlen("ITCM_H"));
        CMBS_SUOTA_LOCAL_FILE_NAME[strlen("ITCM_H")] = '\0';
        SUOTA_INFO("Found CMBS_SUOTA_LOCAL_FILE_NAME = %s\n", CMBS_SUOTA_LOCAL_FILE_NAME);
        RetVal = 0;
    }  
    ...
    else if ( pNamePtr )
    {
        ...
        if ( NameLen == 0 )
        {
            // For the case that the last character is '/'
            pNamePtr--;
            NameLen++;
        }

        pNamePtr++;
        memset(&CMBS_SUOTA_LOCAL_FILE_NAME, 0, MAX_FILE_NAME_LENGTH);

        strncpy((char *)&CMBS_SUOTA_LOCAL_FILE_NAME, pNamePtr, (NameLen - 1));
        SUOTA_INFO("Found CMBS_SUOTA_LOCAL_FILE_NAME = %s\n", CMBS_SUOTA_LOCAL_FILE_NAME);
        RetVal = 0;
    }
    else
    {
        SUOTA_INFO(" SUOTA: GetFileName - File name not found\n");
    }
    return RetVal;
}
/**
...
#ifdef SUOTA_SDU_CHOPPING    
			modifyRangeHttpHdr((char *)pMsgRx->buf, pSession);
#else
...
#endif
		// char *url = (char *)gSuotaMgmtConf.hsVerUrl.st_URL.u8_URL;

		if (u32_NumOfBytesRead == 0)
		{
			// Indicates that this is the first request from the HS.
			// In this case - get file name from the request
			if (GetFileName((char *)pMsgRx->buf) != -1)
			{
				// If file name is found - download the whole file.
				if (g_SUOTAFlags.b_FileDownload)
				{
					...
					if (curl)
					{
						if (tcx_fileOpen(&fpHSBinary, (u8*)CMBS_SUOTA_LOCAL_FILE_NAME, (u8*)"wb") != CMBS_RC_OK)
						{
				...
				// Get information regarding file size
				g_st_FileInfo.u32_FileSize = suota_CheckFileSize();
			}
		}

#ifdef SUOTA_SDU_CHOPPING    
	    if (modifyRangeHttpHdr((char *)pMsgRx->buf, pSession) != -1)
#else
	    modifyRangeHttpHdr((char *)pMsgRx->buf);
#endif
            {
...
void getNextChunk(t_st_GmepAppMsgCmd *pMsgRx, t_Suota_GmepSessionTbl *pSession, t_st_GmepSdu *pHttpresp)
{
    ...
    UNUSED_PARAMETER(pMsgRx);
    memset(&st_Response, 0, sizeof(t_st_httpResponse));
    if ( g_SUOTAFlags.b_HeaderCreatedbyHost )
    {
        suota_ConstructMessage(&st_Response, TRUE, pSession);
    }
    else // In case Target needs to create the header
    {
        suota_ConstructMessage(&st_Response, FALSE, pSession);
    }
        ...
        case GMEP_US_DATA_RECV:
            {
                ...
                if (pSession->SDUChoppingHS)
                {
                    // Handset supports SDU chopping
                    // RECV is received only on new requests
                    g_st_FileInfo.u8_SequenceNum = 0x80;  // reset Sequence number
                    getNextHttpChunk(&msgRx, pSession, &pHttpresp);
                }
                else if (!pSession->ContinueSend)
                {
                    // printf(" Sequence before is %x", g_st_FileInfo.u8_SequenceNum);
                    // new request to be handled
                    if (g_st_FileInfo.u8_SequenceNum == 0xFE)
                    {
                        // reset sequrence number
                        g_st_FileInfo.u8_SequenceNum = 0x81;
                    }
                    else if (g_st_FileInfo.u8_SequenceNum != 0x80)
                    {
                        // increase sequence number
                        g_st_FileInfo.u8_SequenceNum++;
                        // set MSB again, it was maybe cleared on last transmit.
                        g_st_FileInfo.u8_SequenceNum |= 0x80;
                    }
                    // printf(" Sequence after = %x\n",  g_st_FileInfo.u8_SequenceNum   );                   
                    getNextHttpChunk(&msgRx, pSession, &pHttpresp);
                }     
                else
                {
                     // printf(" continue send - modifyRangeHttpHdr \n");    
                     modifyRangeHttpHdr((char *) &msgRx.buf[0], pSession);
                }
#else
                if ( g_SUOTAFlags.b_EarlyChunkPrepare )
                {
                    // In case early chunk prepare option is used, only first chunk is read upon DATA_RECV. Rest of the chunks done already on SEND_ACK
                    if ( !u8_FirstTransDone )
                    {
                        getNextHttpChunk(&msgRx, pSession, &pHttpresp);
                        u8_FirstTransDone = 1;
                    }
        ...
        case GMEP_US_DATA_SEND_ACK:
            {
                ...
                if (pSession->SDUChoppingHS)                
                {
                    if (pSession->ContinueSend)
                    {
                        // Handset supports SDU chopping
                        // RECV is received only on new requests
                        suota_CalculateByteRange(pSession);
                        g_st_FileInfo.u8_SequenceNum++; 
                        getNextChunk(&msgRx, pSession, &pHttpresp);
                    }
                }
                else if (pSession->ContinueSend)
                {
                ...
                if ( g_SUOTAFlags.b_EarlyChunkPrepare )
                {
                    // Now the next packet gets sent here already (not on GMEP_US_DATA_RECV)
                    suota_CalculateByteRange();
                    getNextChunk(&msgRx, pSession, &pHttpresp);
                }
#endif // SUOTA_SDU_CHOPPING
                resAck1 = 1;
    ...
    case GMEP_US_DATA_SEND:
        {
            ...
            if ( g_SUOTAFlags.b_HeaderCreatedbyHost )
            {
                //If Host creates the header, we do not pass file information to the target
                cmbs_dsr_suota_DataSend(NULL, pGmepMsg->appId, pGmepMsg->appSessionId,
                                        (char *)pGmepMsg->buf, pGmepMsg->bufLen, requestId);
            }
            else
            {
                cmbs_dsr_suota_ExtendedDataSend(NULL, pGmepMsg->appId, pGmepMsg->appSessionId,
                                                (char *)pGmepMsg->buf, pGmepMsg->bufLen, requestId, &g_st_FileInfo);
            }
#else
...
char* get_ip(char *host1)
{
    ...
    if(ip==NULL)
    {
        perror("SUOTA: Can't allocate ip\n");
        exit(1);
    }
    memset(ip, 0, iplen + 1);
