...
#define APP_SUOTA_INFO_PRINT(format, ...) 						tcx_WriteLog(SUOTA_MODULE, LOG_LEVEL_INFO, format,  ##__VA_ARGS__ )
#define APP_SUOTA_TRACE_PRINT(format, ...) 						tcx_WriteLog(SUOTA_MODULE, LOG_LEVEL_TRACE, format,  ##__VA_ARGS__ )
#define APP_SUOTA_WARNING_PRINT(format, ...) 					tcx_WriteLog(SUOTA_MODULE, LOG_LEVEL_WARNING, format,  ##__VA_ARGS__ )
#define APP_SUOTA_ERROR_PRINT(format, ...) 						tcx_WriteLog(SUOTA_MODULE, LOG_LEVEL_ERROR, format,  ##__VA_ARGS__ )
#define APP_SUOTA_RT_PRINT(format, ...) 						tcx_WriteLog(SUOTA_MODULE, LOG_LEVEL_REAL_TIME, format,  ##__VA_ARGS__ )
#define APP_SUOTA_PRINT_DATA(pu8_Buffer, u16_Length) 			tcx_WriteLogData(SUOTA_MODULE, LOG_LEVEL_INFO, pu8_Buffer, u16_Length)

#define LOG_GMEP_US
#ifdef LOG_GMEP_US
#define GMEP_LOG(format, ...) 						tcx_WriteLog(CALL_MODULE, LOG_LEVEL_INFO, format,  ##__VA_ARGS__ )
...
//	GMEP_US_PUSH_MODE = CMBS_EV_DSR_SUOTA_PUSH_MODE,
    GMEP_US_UNREG_APP_CB = CMBS_EV_DSR_SUOTA_UNREG_APP_CB,
} t_GmepCmd;
#endif
...
typedef struct
{
	u8 	spare:2;			/*for further development */
	u8 	chop:1;	/*chop bit set if application data   MAX_SDU_SIZE */
	u8 	optionalGrp:2;
	u8 	operationalCode:2;
	u8 	cntrlseText:1;			/* Always set to 1 */
	u8 	gmci:7;				/* Generic Media Context Identifier(gmci). gmci = GMEP session index */
	u8 	seqNo:1;				/* sequence number set, if chop bit is set */
