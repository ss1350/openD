...
#define LOG_INFO(format, ...) 					tcx_WriteLog(CMBS_LIB_MODULE, LOG_LEVEL_INFO, format,  ##__VA_ARGS__ )
#define LOG_TRACE(format, ...) 					tcx_WriteLog(CMBS_LIB_MODULE, LOG_LEVEL_TRACE, format,  ##__VA_ARGS__ )
#define LOG_WARNING(format, ...) 					tcx_WriteLog(CMBS_LIB_MODULE, LOG_LEVEL_WARNING, format,  ##__VA_ARGS__ )
#define LOG_ERROR(format, ...) 					tcx_WriteLog(CMBS_LIB_MODULE, LOG_LEVEL_ERROR, format,  ##__VA_ARGS__ )
#define LOG_RT(format, ...) 						tcx_WriteLog(CMBS_LIB_MODULE, LOG_LEVEL_REAL_TIME, format,  ##__VA_ARGS__ )
#define LOG_DATA(pu8_Buffer, u16_Length) 			tcx_WriteLogData(CMBS_LIB_MODULE, LOG_LEVEL_INFO, pu8_Buffer, u16_Length)
...
void tcx_LogOutputDestroy(void)
{
    ...
    if ( G_tcx_hLoggerThread )
    {
		pthread_cancel( G_tcx_hLoggerThread );
		pthread_join(G_tcx_hLoggerThread, NULL);		 
    }
#endif
...
//                    fflush(g_fpLogFile);
                    WriteLogFile(&g_fpLogFile, pMessage + 5, nLength - 5, loglim * 1000, gpsz_LogFile);
					// Effi - add write to log
					LOG_INFO("%s", pMessage + 5);
                }
                else // console
					LOG_INFO("%s", pMessage);
            }
...
int tcx_LogOpenLogfile(char *psz_LogFile)
{
    ...
    strcpy(gpsz_LogFile, psz_LogFile);
    gpsz_LogFile[sizeof(gpsz_LogFile) - 1] = 0;
                       ...
                       u16 u16_Length, CMBS_LOGGER_DIRECTION_e direction)
{
   ...
   struct timeval  st_TimeVal;
    ...
    u32_Pos += sprintf(tcx_LogPacketString + u32_Pos, "%02d-%02d-%d  %02d:%02d:%02d:%03ld\n", CurrentLocalTime.tm_mday,
    ...
    else
    {
        ...
        else
        {
			...
			// print the IE list only if not raw payload event
			if (FALSE == cmbs_util_RawPayloadEvent(u16_Event))
			{
	...
	if (u32_Pos >= CMBS_MAX_LOG_MESSAGE - 4)
	{
		tcx_LogPacketString[CMBS_MAX_LOG_MESSAGE - 1] = 0;
		tcx_LogPacketString[CMBS_MAX_LOG_MESSAGE - 2] = '.';
		tcx_LogPacketString[CMBS_MAX_LOG_MESSAGE - 3] = '.';
		tcx_LogPacketString[CMBS_MAX_LOG_MESSAGE - 4] = '.';
		tcx_LogPacketString[CMBS_MAX_LOG_MESSAGE - 5] = '\n';
		u32_Pos = CMBS_MAX_LOG_MESSAGE-1;
	}
...
{
	// Effi- remove this function since it is trying to print to log the data taht was sent to uart and that should have been saved in g_u8_tcx_LogOutgoingPacket[]
	// but no function actually insert data to this array so this results in log of "\n" in the logger
	/*
