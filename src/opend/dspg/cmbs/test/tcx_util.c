...
void tcx_USBConfig( u8 u8_Port, u32 u32_BaudRate )
{
    // setup port and uart for control plane
    g_st_UartCfg.u8_Port = u8_Port;
	g_st_UartCfg.u32_BaudRate = u32_BaudRate;
...
u8 tcx_DetectComPort(bool interactiveMode, E_CMBS_DEVTYPE *pe_type)
{
    ...
    *pe_type = CMBS_DEVTYPE_MAX; // no transport type assigned yet
    ...
    for (dwDeviceNumber = 0; dwDeviceNumber < 65536; ++dwDeviceNumber)
    {
        ...
        {
            if ( interactiveMode )
            _tprintf(TEXT("| COM%d: %s\n"), iPortNumber, descData);

            if ( _tcsncmp(szDongleHardwareId, regData, CMBS_USB_DONGLE_HARDWARE_ID_NO_VER_LEN) == 0 )
            {
...
int tcx_gets(char *buffer, int n_Length)
{
	...
	do
	{
		if (fgets(buffer, n_Length, stdin) != NULL)
		{
			if(strlen(buffer) == (n_Length - 1) && (buffer[strlen(buffer) - 1] != '\n'))
			{
			    do
			    	{
					cChar = fgetc(stdin);
...
void tcx_UARTConfig(u8 u8_Port, const char *psz_ComDevName, u32 u32_BaudRate)
{
    // setup port and uart for control plane
    g_st_UartCfg.u8_Port = u8_Port;
    g_st_UartCfg.psz_ComDevName = (const u8 *)psz_ComDevName;
	g_st_UartCfg.u32_BaudRate = u32_BaudRate;	
...
E_CMBS_RC tcx_fileOpen(FILE **pf_file, u8 *pu8_fileName, const u8 *pu8_mode)
{
    ...
    *pf_file = fopen((char *)pu8_fileName, (char *)pu8_mode);
...
E_CMBS_RC tcx_fileRead(FILE *pf_file, u8 *pu8_OutBuf, u32 u32_Offset, u32 u32_Size)
{
    ...
    if ( res != u32_Size )
    {
        CFR_DBG_ERROR("ERROR: Can't read file\n");
        return CMBS_RC_ERROR_GENERAL;
    }
...
E_CMBS_RC tcx_fileWrite(FILE *pf_file, u8 *pu8_InBuf, u32 u32_Offset, u32 u32_Size)
{
    ...
    if ( res != u32_Size )
    {
        CFR_DBG_ERROR("ERROR: Can't write file\n");
        return CMBS_RC_ERROR_GENERAL;
    }
...
E_CMBS_RC tcx_fileDelete(char *pFileName)
{
    ...
    {
        CFR_DBG_ERROR("ERROR - can not delete file RetVal = %d\n", RetVal);
        return CMBS_RC_ERROR_GENERAL;
    }
}
//    ========== tcx_fileSize ===========
